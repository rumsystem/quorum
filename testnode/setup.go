...
func RunNodesWithBootstrap(ctx context.Context, cli Nodecliargs, pidch chan int, n int) (string, []string, string, error) {
	...
	Fork(pidch, KeystorePassword, gocmd, "run", "cmd/main.go", "cmd/utils.go", "-bootstrap", "-listen", fmt.Sprintf("/ip4/0.0.0.0/tcp/%d", bootstrapport), "-apiport", fmt.Sprintf("%d", bootstrapapiport), "-configdir", testconfdir, "-keystoredir", testkeystoredir, "-datadir", testdatadir)
	...
	for i < n {
		...
		Fork(pidch, KeystorePassword, gocmd, "run", "cmd/main.go", "cmd/utils.go", "-peername", peername, "-listen", fmt.Sprintf("/ip4/0.0.0.0/tcp/%d", peerport), "-apiport", fmt.Sprintf("%d", peerapiport), "-peer", bootstrapaddr, "-configdir", testconfdir, "-keystoredir", testpeerkeystoredir, "-datadir", testdatadir, fmt.Sprintf("-rextest=%s", strconv.FormatBool(cli.Rextest)))
