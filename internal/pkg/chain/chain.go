...
func handleReqSignResp(trxMsg TrxMsg) error {
	...
	UpdTrxCons(reqSignResp.ReqTrxId, consensusString)

	//check if we have enough signature
	if trx, err := GetTrx(reqSignResp.ReqTrxId); err != nil {
		return err
	} else {
		if len(trx.Consensus) < GetContext().TrxSignReq {
...
func handleNextBlockResp(trxMsg TrxMsg) error {
	...
	if groupItem, ok := GetContext().Groups[trxMsg.GroupId]; ok {
		if reqNextBlockResp.Response == BLOCK_ON_TOP {
