...
func newBlockProviderScorer(store *peerdata.Store, config *BlockProviderScorerConfig) *BlockProviderScorer {
	...
	batchSize := 10 //TODO: read the value from config
	scorer.maxScore = 1.0
	if batchSize > 0 {
		totalBatches := float64(scorer.config.ProcessedBlocksCap / uint64(batchSize))
...
func (s *BlockProviderScorer) score(pid peer.ID) float64 {
	...
	batchSize := 10 //TODO: read the value from config
	if batchSize > 0 {
		processedBatches := float64(peerData.ProcessedBlocks / uint64(batchSize))
...
func (s *BlockProviderScorer) WeightSorted(
	r *localcrypto.Rand, pids []peer.ID, scoreFn func(pid peer.ID, score float64) float64,
) []peer.ID {
	if len(pids) == 0 {
		return pids
	}
