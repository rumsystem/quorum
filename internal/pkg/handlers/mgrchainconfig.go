...
type TrxAuthModeParams struct {
	...
	TrxAuthMode string `from:"trx_auth_mode" json:"trx_auth_mode" validate:"required,oneof=as_allow as_deny"`
}
...
func MgrChainConfig(params *ChainConfigParams) (*ChainConfigResult, error) {
	...
	if params.Type == strings.ToLower(quorumpb.ChainConfigType_SET_TRX_AUTH_MODE.String()) {
		...
		if err := validate.Struct(&configItem); err != nil {
			return nil, err
		}
		...
		if params.Type == strings.ToLower(quorumpb.ChainConfigType_UPD_ALW_LIST.String()) {
			...
			if dataParams.Action == "add" &&
				group.Item.OwnerPubKey == group.Item.UserSignPubkey {
				return nil, errors.New("GROUP OWNER CAN NOT ADD HIMSELF TO DENY LIST")
			}
...
func getTrxTypeByString(typ string) (quorumpb.TrxType, error) {
	...
	case "SCHEMA":
		return quorumpb.TrxType_SCHEMA, nil
	case "PRODUCER":
		return quorumpb.TrxType_PRODUCER, nil
	case "ANNOUNCE":
	...
	case "USER":
		return quorumpb.TrxType_USER, nil
	case "ASK_PEERID":
		return quorumpb.TrxType_ASK_PEERID, nil
	case "CHAIN_CONFIG":
		return quorumpb.TrxType_CHAIN_CONFIG, nil
	case "APP_CONFIG":
		return quorumpb.TrxType_APP_CONFIG, nil
	default:
