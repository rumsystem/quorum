...
func (d *ChainData) GetReqBlocks(trx *quorumpb.Trx) (requester string, sessionId string, taskId string, fromEpoch int64, reqBlocks int64, blocks []*quorumpb.Block, result quorumpb.ReqBlkResult, err error) {
	...
	if !exist {
		return reqBlockItem.ReqPubkey, reqBlockItem.SessionId, reqBlockItem.TaskId, reqBlockItem.FromEpoch, reqBlockItem.BlksRequested, nil, quorumpb.ReqBlkResult_BLOCK_NOT_FOUND, nil
	}
	...
	for totalBlockPackaged < int(reqBlockItem.BlksRequested) {
		...
		if totalBlockBytes > MAX_BLOCK_IN_RESP_BYTES {
			return reqBlockItem.ReqPubkey, reqBlockItem.SessionId, reqBlockItem.TaskId, reqBlockItem.FromEpoch, reqBlockItem.BlksRequested, blocks, quorumpb.ReqBlkResult_BLOCK_IN_RESP, nil
		}
		...
		//no more and on top
		if !exist {
			return reqBlockItem.ReqPubkey, reqBlockItem.SessionId, reqBlockItem.TaskId, reqBlockItem.FromEpoch, reqBlockItem.BlksRequested, bs, quorumpb.ReqBlkResult_BLOCK_IN_RESP_ON_TOP, nil
		}
		//continue and put more
	}

	return reqBlockItem.ReqPubkey, reqBlockItem.SessionId, reqBlockItem.TaskId, reqBlockItem.FromEpoch, reqBlockItem.BlksRequested, bs, quorumpb.ReqBlkResult_BLOCK_IN_RESP, nil
}
