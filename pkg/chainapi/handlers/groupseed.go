...
func GroupSeedToUrl(version int, urls []string, seed *quorumpb.GroupSeed) (string, error) {
	/*
		urllist := []string{}
		for _, u := range urls {
		...
		//handle genesisBlock
		genesisBlockByt, err := proto.Marshal(seed.GroupItem.GenesisBlock)
		if err != nil {
			return "", err
		}
...
func UrlToGroupSeed(seedurl string) (*quorumpb.GroupSeed, []string, error) {
	/*
