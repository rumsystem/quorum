...
func TestUpdateChainDenyList(t *testing.T) {
	...
	if !found {
		t.Fatalf("should get chain config rule from deny list")
	}
}

func getGroupDenyList(api string, groupId string) ([]handlers.ChainSendTrxRuleListItem, error) {
	url := fmt.Sprintf("/api/v1/group/%s/trx/denylist", groupId)
	_, resp, err := testnode.RequestAPI(api, url, "GET", "")
	if err != nil {
		e := fmt.Errorf("get group deny list for %s failed: %s", groupId, err)
		return nil, e
	}
	...
	var denyList []handlers.ChainSendTrxRuleListItem
	if err := json.Unmarshal(resp, &denyList); err != nil {
